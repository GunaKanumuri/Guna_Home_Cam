...
DESIGNED FOR UNATTENDED OPERATION — runs for months/years without human intervention.
Auto-recovers from: power outages, WiFi drops, DVR reboots, script crashes.

NEW IN v9
═════════
[v9-1]  External config.json — never edit Python to change settings
[v9-2]  Offline notification queue — buffers alerts during WiFi outage, replays when back
[v9-3]  Multi-frame burst — sends 3 frames to Gemini instead of 1 (motion visible in IR)
[v9-4]  Context injection — tells Gemini what was detected 30s ago on same camera
[v9-5]  Family member descriptions — Gemini distinguishes family from strangers
[v9-6]  Cross-camera event correlation — person at gate → ground floor = same person
[v9-7]  Weekly summary — Sunday night trend analysis of the week
[v9-8]  Mock mode — test without live cameras using saved debug images
[v9-9]  Gemini offline fallback — saves motion event even when API unreachable
[v9-10] WiFi resilience — retries indefinitely, never gives up

CARRIED FROM v8: all 17 fixes preserved.

Dependencies: requests pillow numpy ffmpeg (system)
Run: bash start_icmob.sh  (recommended)
Test: python icmob_ai.py --test-cam cam1
Mock: python icmob_ai.py --mock
"""
...
def _load_config() -> dict:
    """Load config.json from known locations. Merge with defaults."""
    defaults = {
        "cameras": [],
        "family_members": [],
        "gemini": {
    ...
    print("⚠️  No config.json found — using defaults + env vars")
    return defaults

CFG = _load_config()

# ── Extract config into module-level constants ────────────────────────
CAMERAS           = CFG["cameras"]
...
EVENT_PREFIX = {
    "person": "person_detected", "family": "person_detected",
    "intruder": "intruder_detected", "package": "package_detected",
    "vehicle": "vehicle_detected", "animal": "animal_detected",
    "unknown": "unknown_activity", "nothing": "nothing_notable",
}
...
# ═══════════════════════════════════════════════════════════════════════
#  FRAME GRABBER — multi-frame burst [v9-3]
# ═══════════════════════════════════════════════════════════════════════

def grab_frame(camera: dict) -> Image.Image | None:
    try:
        cmd = [
            "ffmpeg", "-threads", "1", "-rtsp_transport", "tcp",
            "-i", camera["rtsp_url"],
            "-vframes", "1", "-q:v", "3",
        ...
        log.warning(f"[{camera['name']}] ffmpeg fail ({proc.returncode}): {stderr}")
        return None
...
def detect_motion(cam_id: str, img: Image.Image,
    ...
    if flip:
        ...
        log.info(f"[{cam_id}] IR transition — skip")
        return False, 0.0
...
# ═══════════════════════════════════════════════════════════════════════
#  CONTEXT TRACKER — [v9-4] per-camera last event for Gemini context
# ═══════════════════════════════════════════════════════════════════════

_last_events: dict[str, dict] = {}   # cam_id → {event, person, summary, time}
_last_ev_lock = Lock()

# ═══════════════════════════════════════════════════════════════════════
#  EVENT CORRELATION — [v9-6] cross-camera tracking
# ═══════════════════════════════════════════════════════════════════════
...
def _check_correlation(camera: dict, result: dict) -> str | None:
    """Check if person was seen at another camera within 60s."""
    ...
    with _corr_lock:
        for det in reversed(_recent_detections):
            ...
            if det["event"] in ("person_detected", "intruder_detected"):
                return f"Same person likely seen at {det['cam_name']} {int(now - det['time'])}s ago"
    return None

# ═══════════════════════════════════════════════════════════════════════
#  GEMINI AI — [v9-3] multi-frame, [v9-4] context, [v9-5] family
# ═══════════════════════════════════════════════════════════════════════
...
HOW TO READ IR:
- People = bright white/grey shapes against dark background
- Eyes and skin reflect IR — look for bright face/hand shapes
- Clothing: light grey = thin/white, dark grey = thick/dark
- Do NOT dismiss shapes as shadows — human silhouette = person"""
    elif is_night:
        img_ctx = "IMAGE TYPE: Low-light night capture."
    else:
        img_ctx = "IMAGE TYPE: Normal daylight, colors accurate."
    ...
    with _last_ev_lock:
        ...
        if prev and time.time() - prev.get("time", 0) < 120:
            ...
            prev_ctx = f"\nPREVIOUS DETECTION ({ago}s ago on this camera): {prev.get('event','')} — {prev.get('summary','')}"

    # [v9-3] Multi-frame note
    frame_note = "You are seeing MULTIPLE frames captured over ~1 second. Look for MOVEMENT between frames.\n" if MULTI_FRAMES > 1 else ""

    return f"""{ctx}
{img_ctx}
{prev_ctx}

{frame_note}TASK: You are a security camera AI. Describe EXACTLY what you see.

IMPORTANT — The house owner is NOT home. Nobody should be inside or near the house.
Any person you see is important. Describe them in FULL DETAIL.

Reply in EXACTLY this format (6 lines, no extra text):
...
ALERT: <yes | no>
SUMMARY: <what is happening, plain English, max 15 words>
PERSON: <DETAILED description — see rules below>
PRIORITY: <urgent | high | medium | low>
CONFIDENCE: <0 to 100>

PERSON DESCRIPTION RULES (most important field):
- If you see a person, describe ALL of these that you can see:
  1. Gender: man/woman/boy/girl/person (if unclear)
  2. Approximate age: young/middle-aged/elderly
  3. Body build: slim/medium/heavy
  4. Clothing: shirt color, pants/saree/lungi, any visible patterns
  5. What they are doing: standing/walking/sitting/running/bending
  6. Where exactly: near gate/at door/on stairs/on road/in balcony
  7. Direction: entering/leaving/passing by/standing still
- Example GOOD: "middle-aged man, medium build, white shirt and dark pants, walking towards main gate from road"
- Example GOOD: "young woman, slim, light blue saree, standing at front door, looking at phone"
- Example GOOD (IR/night): "person, medium build, light colored top, standing near gate, facing the house"
- Example BAD: "person detected" or "someone is there" (too vague!)
- If night/IR and clothing is unclear: describe build, posture, position, and movement instead
- If no person: write "none"
OTHER RULES:
- person_detected = ANY person visible (walking, standing, passing by)
- intruder_detected = ONLY if doing something suspicious (climbing wall, hiding, trying locks, breaking things)
- Walking or standing normally is NOT intruder, even at night
- ALERT yes for person/intruder/package/vehicle/unknown. ALERT no for nothing/animal.
- PRIORITY urgent ONLY for intruder with suspicious behaviour
- SUMMARY = short notification text. Example: "Man in white shirt walking towards main gate"""
...
def _call_api(model: str, camera: dict,
    ...
    log.info(f"[{camera['name']}] GEMINI ({len(frames_bytes)}fr/{total_kb}KB): {text.strip()[:250]}")
    ...
    with _gem_sem:
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    log.warning(f"[{camera['name']}] Gemini timeout — retry in 3s")
                    time.sleep(3)
                else:
                    return None
                ...
                if "model_not_found" in err:
                    with _model_lock:
                        if _model != GEMINI_FALLBACK:
                            log.warning(f"Model gone — fallback to {GEMINI_FALLBACK}")
                    ...
                    log.warning(f"[{camera['name']}] 429 — retry in 10s")
                    time.sleep(10)
                else:
                    return None
...
def check_disk(warn=True) -> float:
    try:
        ...
        if warn and free < MIN_DISK_MB:
            _ntfy_simple("iCMOB — Low Disk", f"⚠️ {free:.0f} MB free", tags="warning", priority="4")
        return free
    except Exception:
        return 9999.0
...
def _queue_notification(data: dict):
    ...
    try:
        ...
        log.info(f"📥 Notification queued for later (offline)")
    except Exception as e:
        log.error(f"Queue error: {e}")


def _replay_pending():
    """[v9-2] Replay buffered notifications when WiFi is back."""
    ...
    try:
        ...
        for line in lines:
            try:
                ...
                failed_lines.append(line)
                break   # WiFi still down, stop trying
            except Exception:
                failed_lines.append(line)

        # Rewrite file with only failed ones
        with _pending_lock:
            with open(PENDING_FILE, "w") as f:
                f.writelines(failed_lines)

        if sent:
            log.info(f"📤 Replayed {sent} queued notifications ({len(failed_lines)} remaining)")

    except Exception as e:
        log.error(f"Replay error: {e}")

# ═══════════════════════════════════════════════════════════════════════
#  NOTIFICATIONS — with offline buffering
# ═══════════════════════════════════════════════════════════════════════
...
def _send_ntfy(camera: dict, result: dict,
    ...
    headers = {
        **_ntfy_auth(),
        "Title": camera["name"], "Tags": cfg["emoji"],
        "Priority": str(cfg["ntfy_pri"]), "Message": body,
    }
    ...
    for attempt in range(2):
        try:
            if img:
                ...
                r = requests.post(
                    f"{NTFY_SERVER}/{NTFY_TOPIC}", headers=headers, timeout=10
                )
            if r.status_code in (200, 201):
            ...
            log.warning(f"Ntfy error (att {attempt+1}): {e}")
        if attempt == 0:
            time.sleep(3)

    # [v9-2] Queue for later replay
    log.warning(f"❌ Ntfy failed — queuing for later")
    _queue_notification({
        "title": camera["name"], "tags": cfg["emoji"],
    ...
    try:
        ...
        if img:
            ...
            requests.post(f"{base}/sendMessage",
                json={"chat_id": TELE_CHAT_ID, "text": text, "parse_mode": "Markdown"}, timeout=10)
    except Exception as e:
...
def _ntfy_simple(title: str, msg: str, tags="bell", priority="2"):
    ...
    try:
        requests.post(f"{NTFY_SERVER}/{NTFY_TOPIC}", headers={
            **_ntfy_auth(), "Title": title, "Tags": tags,
...
def save_event(camera: dict, result: dict, score: float,
    ...
    rec = {
        "event_id": eid, "timestamp": now.isoformat(),
        "camera_id": camera["id"], "camera_name": camera["name"],
        "event": result.get("EVENT", ""), "alert": result.get("ALERT", "no"),
        "summary": result.get("SUMMARY", ""),
        "person": result.get("PERSON", "none"),
        "priority": result.get("PRIORITY", "low"),
        "confidence": result.get("CONFIDENCE", 0),
        "motion_score": score, "night": is_night, "ir_mode": ir,
    }
    try:
        with _elog_lock:
...
def trim_log():
    ...
    try:
        ...
        if dropped:
            ...
            log.info(f"Event log trimmed: -{dropped} +{kept}")
        else:
            os.remove(tmp)
    except Exception as e:
        log.error(f"trim: {e}")
...
def _count_events(date_prefix: str) -> dict:
    """Count events for a date prefix (YYYY-MM-DD or YYYY-MM range)."""
            ...
            for line in f:
                try:
                    ...
                    if evt == "person_detected":
                        ...
                        counts["vehicle"] += 1
                    elif evt == "animal_detected":
                        counts["animal"] += 1
                    elif evt == "unknown_activity":
                        counts["unknown"] += 1
                except Exception:
                    continue
    except Exception:
...
def send_daily_summary():
    ...
    log.info(f"Daily summary: {body}")


def send_weekly_summary():
    """[v9-7] Sunday night weekly trend summary."""
    if not NTFY_ENABLED:
        return
    ...
    if totals["total"] == 0:
        _ntfy_simple("📈 Weekly Summary", "Very quiet week — 0 events", tags="chart_with_upwards_trend")
        return
    ...
    try:
        ...
        if mins > 10:
            ...
            _ntfy_simple(
                "⚡ iCMOB — Was Offline",
                f"Offline for {dur}\n"
                f"Last: {last.strftime('%b %d %I:%M %p')}\n"
                f"Back: {datetime.now().strftime('%b %d %I:%M %p')}\n"
                f"Cause: power outage / crash / WiFi loss",
                tags="electric_plug", priority="4"
            )
            log.warning(f"Outage detected — offline {dur}")
        else:
    ...
    try:
        ...
        if plug == "UNPLUGGED":
            _ntfy_simple("🔌 Phone Unplugged!",
                         f"Battery {pct}%. Plug it back in!",
                         tags="electric_plug", priority="4")
        elif pct < 15:
            _ntfy_simple("🪫 Low Battery",
                         f"{pct}% — check charger", tags="battery", priority="3")
...
# ═══════════════════════════════════════════════════════════════════════
#  CAMERA WATCHER — with multi-frame, context, correlation
# ═══════════════════════════════════════════════════════════════════════

class CameraWatcher:
    def __init__(self, camera, cooldown=None):
        self.cam       = camera
    ...
    def _process(self, img, score, is_night, ir):
        self.frames += 1

        # Fast motion = skip loiter, immediate multi-frame burst
        if score >= self.thresh * 3:
            ...
            if not m2:
                log.info(f"[{self.name}] Loiter: motion gone")
                return
            # Multi-frame burst
            frames = [img2] + [f for f in [grab_frame(self.cam) for _ in range(MULTI_FRAMES - 1)] if f]
            score = s2

        if not frames:
            return

        # Validate primary frame
        valid, reason = validate_frame(frames[0])
        if not valid:
            log.info(f"[{self.name}] Frame rejected: {reason}")
            return

        # [v9-9] Try Gemini — if offline, save event without AI
        result = analyze(self.cam, frames, is_night, ir)
        if not result:
            # Gemini unreachable — save basic motion event
            result = {
    ...
    def run(self):
        ...
        while not _shutdown.is_set():
            try:
                ...
                self.cfails = 0
                ir = is_ir_mode(img)
                motion, score = detect_motion(self.cid, img, self.thresh, self.cam)
...
def run_test(cam_id):
    ...
    with open(dp, "wb") as f:
        f.write(bts)
    print(f"  📸 Debug image: {dp} ({len(bts)//1024}KB)")
    print(f"{'═'*50}\n")


def run_mock():
    """[v9-8] Mock mode — replay saved debug images through Gemini."""
    images = glob.glob(os.path.join(DEBUG_DIR, "*.jpg"))
    if not images:
        print("No debug images found. Run --test-cam first to generate some.")
        sys.exit(1)

    images = sorted(images)[-5:]   # last 5
    print(f"\n{'═'*50}")
    ...
    cam = CAMERAS[0] if CAMERAS else {"id": "mock", "name": "Mock", "location": "test"}
...
def check_config() -> bool:
    ...
    if not GEMINI_API_KEY:
        ...
        log.error("   Set in config.json or: export GEMINI_API_KEY=your_key")
        ok = False
...
def check_wakelock():
    try:
        r = subprocess.run(["pgrep", "-f", "termux-wake-lock"],
...
def main():
    ...
    parser.add_argument("--test-cam", metavar="ID", help="Test camera: --test-cam cam1")
    parser.add_argument("--mock", action="store_true", help="Mock mode with saved images")
    ...
    ╔══════════════════════════════════════════╗
    ║     iCMOB AI Monitor v9                  ║
    ║     Guna's Home Security                 ║
    ║     Unattended · Self-Healing · 24/7     ║
    ╚══════════════════════════════════════════╝
    """)

    if not check_config():
        sys.exit(42)   # 42 = config error, wrapper won't restart

    if not ping_gemini():
        log.error("Gemini failed — will retry on restart")
        sys.exit(1)    # 1 = wrapper will restart
    ...
    log.info(f"  Multi-frame : {MULTI_FRAMES} frames × {MULTI_INTERVAL_MS}ms")
    log.info(f"  Family      : {len(FAMILY_MEMBERS)} members defined")
    log.info(f"  Ntfy        : {'✅ ' + NTFY_TOPIC if NTFY_ENABLED else '❌'}")
    log.info(f"  Heartbeat   : every {HEARTBEAT_SEC//3600}h")
    log.info(f"  Disk        : {free:.0f} MB")
    log.info(f"  Config      : config.json")
    log.info("─" * 50)

    # Replay any pending notifications from previous WiFi outage
    _replay_pending()
    ...
    Thread(target=health_monitor, args=(enabled,), daemon=True, name="health").start()
    ...
    _ntfy_simple("iCMOB Stopped 🛑", "Clean shutdown. Wrapper will restart if needed.",
